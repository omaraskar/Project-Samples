...
!---------------------------------------------------------------------!
!               ADVECTION TERM FOR THE FREE SURFACE PROBLEM           !
!                              March 2014                             !
!---------------------------------------------------------------------!
!wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww!

      SUBROUTINE FS_Advection(Am0,Am1,Am2,Am3,AmT,AmB,AmG,    &
                              uu,vv,ww,pp,                    &
...
!  | --> pp      |(N_CELL,NZ)| Pressure variable at the old time   |  ! 
!  |_____________|___________|_____________________________________|  !
...
!     Gradient of the velocity component
      call grandientLSM(dphidx,dphidy,dphidsig,phi,No_cp,nbe,sig)

!     _________________________________________________
!     Gradient of the pressure p^(n)
      call grandientLSM(dpdx,dpdy,dpdz,pp,No_cp,nbe,sig)  
...
!           ___________________________________________________
!           Top neighbor  
            UiT(i,k) =  0.5d0*(ww(i,k)+ww(i,k+1))*areaCell(i)
!           ___________________________________________________
!           Bottom neighbor
            UiB(i,k) = -0.5d0*(ww(i,k)+ww(i,k-1))*areaCell(i)
        enddo
      ENDDO
